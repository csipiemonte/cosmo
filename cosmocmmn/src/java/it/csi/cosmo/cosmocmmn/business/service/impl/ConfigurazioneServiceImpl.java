...
import it.csi.cosmo.cosmocmmn.config.ErrorMessages;
import it.csi.cosmo.cosmocmmn.config.ParametriApplicativo;
import it.csi.cosmo.cosmocmmn.config.ParametriApplicativo.ExposurePolicy;
import it.csi.cosmo.cosmocmmn.integration.rest.CosmoAuthorizationConfigurazioneParametriClient;
...
implements ConfigurazioneService, InitializingBean, Monitorable {
  ...
  private static final CosmoLogger LOGGER =
      LoggerFactory.getLogger(LogCategory.BUSINESS_LOG_CATEGORY,
          "ConfigurazioneServiceImpl");
  ...
  @Override
  public boolean isReadyConfigProps() {
    try {
  ...
  private void verificaConfigurazionePacchetti() {
    ...
    boolean isReadyConfigProps = isReadyConfigProps();
    ...
    for (ParametriApplicativo parametro : ParametriApplicativo.values()) {
      verificaParametro(parametro, map, errorMessages, isReadyConfigProps);
    }
  ...
  private void verificaParametro(ParametriApplicativo parametro,
      Map<ParametriApplicativo, ConfigurazioneDTO> map, List<String> errorMessages, boolean isReadyConfigProps) {
  ...
  private ConfigurazioneDTO getFromCosmoAuthorization(String variazioneChiave) {
    ...
    CosmoAuthorizationConfigurazioneParametriClient client =
        (CosmoAuthorizationConfigurazioneParametriClient) SpringApplicationContextHelper
        .getBean(CosmoAuthorizationConfigurazioneParametriClient.class);

    ParametriResponse parametriResponse = client
