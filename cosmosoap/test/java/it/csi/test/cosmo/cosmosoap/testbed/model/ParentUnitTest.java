...
package it.csi.test.cosmo.cosmosoap.testbed.model;
...
import it.csi.cosmo.cosmosoap.util.listener.AppServletContextListener;
...
public abstract class ParentUnitTest implements ApplicationContextAware {
  ...
  protected void expect(Runnable task, Class<? extends Exception> clazz) {
    try {
      task.run();
      ...
      if (clazz.isAssignableFrom(e.getClass())) {
        log("succesfully got " + e.getClass().getName() + " (expected "
            + clazz.getName() + ")");
        logger.error(e);
        return;
      }
  ...
  protected void sleep(long millis) {
    log("sleeping " + millis + " milliseconds ...");
    try {
      Thread.sleep(millis);
