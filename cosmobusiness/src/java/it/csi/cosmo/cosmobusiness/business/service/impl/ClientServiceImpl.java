...
import it.csi.cosmo.cosmobusiness.business.service.ClientService;
...
import it.csi.cosmo.cosmobusiness.security.Scopes;
...
public class ClientServiceImpl implements ClientService {
  ...
  private static ClientInfoDTO clientSistema = ClientInfoDTO.builder()
      .withNome("COSMOBUSINESS")
      .withCodice("COSMOBUSINESS")
      .withScopes(Collections.emptyList())
      .withAnonimo(false)
      .build();
  //@formatter:on
  ...
  protected ClientInfoDTO caricaClientInfo(HttpServletRequest request) {
    ...
    if (!StringUtils.isEmpty(userAuthHeader)) {
      AuthenticationToken<ClientInfoDTO> authToken = new CosmoJWTTokenDecoder(
          ...
          LoggerConstants.ROOT_LOG_CATEGORY).decodeAndVerify(userAuthHeader, ClientInfoDTO.class);
