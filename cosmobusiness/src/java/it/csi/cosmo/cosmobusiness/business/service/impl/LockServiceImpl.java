...
import it.csi.cosmo.cosmobusiness.integration.mapper.LockMapper;
import it.csi.cosmo.cosmobusiness.integration.repository.CosmoTLockRepository;
...
public class LockServiceImpl implements LockService {
  ...
  private Lock map(CosmoTLock entity) {
    var dto = lockMapper.toDTO(entity);
    dto.setUtente(
        lockMapper.toDTO(estraiUtenteDaDatiTecnici(entity.getUtenteInserimento()).orElse(null)));
    return dto;
  }

  @Override
  @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
  public Lock getLock(String codiceRisorsa) {
    ValidationUtils.require(codiceRisorsa, "codiceRisorsa");

    CosmoTLock lock = findActiveLock(codiceRisorsa, true).orElseGet(() -> {
      CosmoTLock emptyLock = new CosmoTLock();
      emptyLock.setCodiceRisorsa(codiceRisorsa);
      return emptyLock;
    });
  ...
  @Override
  @Transactional(readOnly = true, propagation = Propagation.REQUIRED)
  public void checkLock(String codiceRisorsa, boolean required) {

    CosmoTLock lock = findActiveLock(codiceRisorsa, true).orElse(null);
    if (lock == null) {
      if (required) {
        throw new ConflictException(
            "Lock sulla risorsa " + codiceRisorsa + " richiesto ma non presente");
      }
      return;
    }

    Optional<String> providedOwnerCode = getResourceLockHeader();
    if (providedOwnerCode.isEmpty()) {
      throw new ConflictException("Lock sulla risorsa richiesto ma non fornito");
    }

    if (!lock.getCodiceOwner().equals(providedOwnerCode.get())) {
      throw new ConflictException("Lock sulla risorsa posseduto da altro utente o altra sessione");
    }
  }

  @Override
  @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
  public Lock postLock(AcquisizioneLockRequest body) {
    ValidationUtils.require(body, "body");
    ValidationUtils.validaAnnotations(body);

    /*
     * sincronizzo sulla classe, solo una operazione di post/delete consentita alla volta.
     *
     * questo non garantisce assenza di race conditions su diverse istanze dell'applicativo ma
     * contribuisce a ridurle sulla stessa istanza.
     *
     * le race conditions su diversi applicativi sono individuate da unique key sulla tabella e
     * gestite applicativamente.
     */
    var req = new LockAcquisitionRequest();
    req.codiceOwner = body.getCodiceOwner();
    req.codiceRisorsa = body.getCodiceRisorsa();
    req.durata = body.getDurata();
    req.timeout = 5000L;

    var res = withOptionalLocalThreadLocking(() -> attemptAcquisition(req));

    if (!res.acquired) {
      throw new ConflictException(res.reason);
    }
  ...
  @Override
  @Transactional(readOnly = false, propagation = Propagation.REQUIRED)
  public boolean deleteLock(RilascioLockRequest body) {
    ValidationUtils.require(body, "body");
    ValidationUtils.validaAnnotations(body);
     ...
     * gestite applicativamente.
     */
    var req = new CosmoTLock();
    req.setCodiceOwner(body.getCodiceOwner());
    req.setCodiceRisorsa(body.getCodiceRisorsa());
    return withOptionalLocalThreadLocking(() -> attemptRelease(req));
  }
  ...
  private LockAcquisitionAttemptResult attemptAcquisition(LockAcquisitionRequest body) {
    ...
    if (result.getError() instanceof org.springframework.dao.DataIntegrityViolationException) {
      // race condition nell'inserimento del lock. il thread non ha acquisito il lock
      var out = new LockAcquisitionAttemptResult();
      out.acquired = false;
      out.lock = null;
  ...
  private Optional<String> getResourceLockHeader() {
    var req = RequestUtils.getCurrentRequest();
    if (req.isEmpty()) {
      return Optional.empty();
    }
    String headerName = "X-Resource-Lock";
    return RequestUtils.streamHeaders(req.get(), headerName).findFirst();
  }
