...
package it.csi.cosmo.cosmobusiness.util.listener;
...
import it.csi.cosmo.cosmobusiness.business.rest.impl.ParentApiImpl;
...
public class SpringApplicationContextHelper implements ApplicationContextAware {
  ...
  private static final CosmoLogger LOGGER =
      LoggerFactory.getLogger(LogCategory.UTIL_LOG_CATEGORY, "SpringApplicationContextHelper");
  ...
  public static <T> T getBean(Class<T> class1) {
    ...
    T bean = appContext.containsBean(beanName) ? (T) appContext.getBean(beanName)
        : (T) appContext.getBean(beanName.substring(0, 1).toLowerCase() + beanName.substring(1));
