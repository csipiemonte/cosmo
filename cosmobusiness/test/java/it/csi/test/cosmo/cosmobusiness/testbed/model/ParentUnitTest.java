...
import it.csi.cosmo.cosmobusiness.util.listener.AppServletContextListener;
...
public abstract class ParentUnitTest implements ApplicationContextAware {
  ...
  protected void dump(Object... data) {
    ...
    for (Object s : data) {
      try {
        ...
        logger.warn("error serializing data", e);
        dstr.append(s.toString()).append(" - (CANNOT SERIALIZE: " + e.getMessage() + ") \n");
      }
    }
  ...
  protected <T extends Exception> T expect(Runnable task, Class<T> clazz) {
    return expect(task, clazz, null);
  }
  ...
  protected void sleep(long ms) {
    try {
      ...
      throw new InternalServerException("SLEEP INTERRUPTED", e);
    }
  }

  @SuppressWarnings("unchecked")
  protected <T extends Exception> T expect(Runnable task, Class<T> clazz,
      Consumer<T> exceptionCheck) {
    try {
      task.run();
      throw new AssertionFailedError(
          "Expected exception " + clazz.getName() + " but got no exceptions");
    } catch (Exception e) {
      if (clazz.isAssignableFrom(e.getClass())) {
        log("succesfully got " + e.getClass().getName() + " (expected " + clazz.getName() + ")");
        // logger.error(e);
        var typed = (T) e;
        if (exceptionCheck != null) {
          exceptionCheck.accept(typed);
        }
        return typed;
      }
      throw new AssertionFailedError(
          "Expected exception " + clazz.getName() + " but got " + e.getClass().getName());
    }
  }
