...
public class MailServiceImpl implements MailService, DisposableBean {
  ...
  private boolean isEnabled() {
    return configurazioneService.requireConfig(ParametriApplicativo.MAIL_ENABLE).asBool();
  }
  ...
  public Future<CosmoMailDeliveryResult> inviaMailAssistenza(String subject, String text) {
    ...
    subject = "[" + Constants.COMPONENT_DESCRIPTION + " "
        + (ConfigurazioneServiceImpl.getCurrentEnvironment()) + "] " + subject;
  ...
  @Override
  public Future<CosmoMailDeliveryResult> inviaMailOtp(String otp, String mailTo) {
    String method = "inviaMailAssistenza";
    logger.debug(method, "invio della mail per richiesta otp a " + mailTo);

    //@formatter:off
    CosmoMail mail = CosmoMail.builder()
        .withSubject("OTP autorizzativo per firma elettronica avanzata (FEA)")
        .withText("Il tuo codice OTP &egrave <strong>"+otp+"</strong><br/><br/>Per autorizzare la FEA inserire il codice ricevuto nel campo dell'OTP.<br/><br/><em>Questa &egrave un'email generata automaticamente, si prega di non rispondere al presente messaggio.</em>")
        .withFrom(configurazioneService.requireConfig(ParametriApplicativo.MAIL_DEFAULT_FROM).asString())
        .withFromName("COSMO")
        .withTo(Arrays.asList(mailTo))
        .build();
    //@formatter:on
