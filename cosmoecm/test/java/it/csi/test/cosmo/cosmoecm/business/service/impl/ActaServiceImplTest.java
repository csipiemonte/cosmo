...
import it.csi.test.cosmo.cosmoecm.business.service.impl.ActaServiceImplTest.TestConfig;
...
@Transactional
public class ActaServiceImplTest extends ParentIntegrationTest {
  ...
  private void setUpMock() {
    ...
    when(mockCosmoSoapActaFeignClient.getDocumentiFisiciByidDocumentoSemplice(any(), any()))
        .thenReturn(getDocumentiFisici());
    when(mockCosmoSoapActaFeignClient.getRicercaPerIndiceClassificazioneEstesa(any(), any()))
        .thenReturn("objectID");
    when(mockCosmoSoapActaFeignClient.getDocumentiSempliciPageable(any(), any(), any()))
        .thenReturn(getDocumentiSemplici());
    when(mockCosmoSoapActaFeignClient.getDocumentiSempliciMap(any(), any()))
        .thenReturn(getDocumentiSempliciMap());
    when(mockCosmoSoapActaFeignClient.getDocumentiFisiciMap(any(), any()))
        .thenReturn(new DocumentiFisiciMap());
    when(mockCosmoSoapActaFeignClient.getContenutoPrimarioId(any(), any()))
        .thenReturn(new ContenutoDocumentoFisico());
    when(mockCosmoSoapActaFeignClient.getRicercaTitolario(any(), any()))
        .thenReturn(new Titolario());
    when(mockCosmoSoapActaFeignClient.ricercaAlberaturaVociPageable(any(), any(), any(), any()))
        .thenReturn(new VociTitolario());
  ...
  public void ricercaIdByIndiceClassificazioneEstesaAggregazione() {
    setUpMock();
    var esito = actaService.findIdByIndiceClassificazioneEstesaAggregazione("identita",
