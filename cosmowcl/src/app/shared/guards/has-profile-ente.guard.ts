...
@Injectable()
export class HasProfileEnteGuard implements CanActivate {
  constructor(
    ...
    return this.authService.getCurrentUser().pipe(
      map(user => {
        ...
        if (user.anonimo) {
          ...
          result = false;
        } else if (!Utils.hasValidProfileEnte(user)) {
          if (Utils.hasValidProfile(user)) {
            this.logger.info('ente non selezionato. redirigo a pagina di amministrazione');
            this.redirectAdmin();
          } else {
