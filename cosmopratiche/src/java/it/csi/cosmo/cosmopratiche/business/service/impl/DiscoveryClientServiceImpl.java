...
import it.csi.cosmo.cosmopratiche.business.service.DiscoveryClientService;
import it.csi.cosmo.cosmopratiche.business.service.MonitoringService;
...
@Service
public class DiscoveryClientServiceImpl implements DiscoveryClientService, InitializingBean, DisposableBean {
  ...
  private boolean isEnabled() {
    return configurazioneService.requireConfig(ParametriApplicativo.DISCOVERY_CLIENT_ENABLE).asBool();
  }

  @Override
  public void afterPropertiesSet() throws Exception {
    AsyncUtils.when("INIT_DISCOVERY_SERVICE", LoggerConstants.COSMOPRATICHE_ROOT_LOG_CATEGORY,
  ...
  private synchronized DiscoveryClient getClient() {
    ...
    if (client == null) {
      ...
      client = DiscoveryClient.builder()
          ...
          .withDiscoveryServerEndpoint(
              configurazioneService.requireConfig(ParametriApplicativo.DISCOVERY_SERVER_LOCATION).asURI())
          .withServiceConfiguration(DiscoveryServiceConfiguration.builder()
              .withName(Constants.DISCOVERY.SERVICE_NAME).withDescription(Constants.COMPONENT_DESCRIPTION)
              .withRoute(Constants.DISCOVERY.SERVICE_ROUTE).build())
          .withInstanceConfiguration(DiscoveryInstanceConfiguration.builder()
              .withHeartBeatInterval(configurazioneService
                  .requireConfig(ParametriApplicativo.DISCOVERY_CLIENT_HEARTBEAT_INTERVAL)
                  .asInteger())
              .withLocation(configurazioneService
                  .requireConfig(ParametriApplicativo.DISCOVERY_CLIENT_LOCATION).asURI())
              .build())
          .withUsername(configurazioneService.requireConfig(ParametriApplicativo.DISCOVERY_CLIENT_USERNAME)
              .asString())
          .withPassword(configurazioneService.requireConfig(ParametriApplicativo.DISCOVERY_CLIENT_PASSWORD)
              .asString())
          .build();
      // @formatter:on
    }
