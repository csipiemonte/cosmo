...
import it.csi.cosmo.cosmonotifications.dto.exceptions.NotReadyException;
...
public class ClusterServiceImpl implements ClusterService, InitializingBean {
  ...
  private static final String MASTER_INSTANCE_RESOURCE_CODE = "NOTIFICATIONS_MASTER_INSTANCE_LEASE";

  private static final int MASTER_INSTANCE_LEASE_INTERVAL = 5 * 1000;

  private CosmoLogger logger =
      LoggerFactory.getLogger(LogCategory.COSMONOTIFICATIONS_BUSINESS_LOG_CATEGORY, "ClusterServiceImpl");
  ...
  private InfinispanFingerprintHelper fingerprintHelper =
      new InfinispanFingerprintHelper(LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY);

  @Override
  public void afterPropertiesSet() throws Exception {
    AsyncUtils.when("INIT_CLUSTER_SERVICE",
        LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY,
  ...
  private void initNegotiator() {
    ...
    lockManager =
        new InfinispanLockManager(connector.getCache(),
            LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY);
    ...
    masterInstanceNegotiator =
        new InfinispanNegotiationManager(MASTER_INSTANCE_RESOURCE_CODE, TimeUnit.MILLISECONDS,
            MASTER_INSTANCE_LEASE_INTERVAL, lockManager,
            LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY);
