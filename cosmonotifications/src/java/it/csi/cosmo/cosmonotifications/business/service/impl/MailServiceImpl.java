...
public class MailServiceImpl implements MailService, DisposableBean {
  ...
  private static final CosmoLogger logger =
      LoggerFactory.getLogger(LogCategory.COSMONOTIFICATIONS_MAIL_LOG_CATEGORY, "MailServiceImpl");
  ...
  private synchronized ICosmoMailer getMailer() {
    if (this.cosmoMailer == null) {
      ...
      this.cosmoMailer = CosmoMailerFactory.defaultMailer()
          ...
          .withLoggingPrefix(LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY)
          .build();
      //@formatter:on
    }
  ...
  @Override
  public Future<CosmoMailDeliveryResult> inviaNotifiche(String subject, String text, String recipient) {
    String method = "inviaNotificheMail";
    logger.debug(method, "invio della mail " + subject);

    subject = "["
        + (ConfigurazioneServiceImpl.getCurrentEnvironment()) + "] - Nuova notifica - [ " + subject + " ]";
