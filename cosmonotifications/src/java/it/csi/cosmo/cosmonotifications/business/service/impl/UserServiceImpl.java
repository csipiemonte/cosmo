...
public class UserServiceImpl implements UserService {
  ...
  private CosmoLogger logger = LoggerFactory
      .getLogger(LogCategory.COSMONOTIFICATIONS_BUSINESS_LOG_CATEGORY, "UserServiceImpl");
   ...
   * {@inheritDoc}
   *
   * Se nessun utente e' correntemente profilato (utente anonimo), viene ritornato un oggetto
   * UserInfo costruito appositamente per l'utente anonimo, avente una lista di Authorities vuota e
   * un codice ruolo pari alla costante Constants.RUOLI.ANONIMO; In questo modo posso utilizzare
   * l'entita' 'utente corrente' coerentemente in tutto l'applicativo, sapendo che non sara' mai
   * NULL, semplificando notevolmente i controlli delle autorizzazioni basati sulle Authorities
   */
  @Override
  ...
  protected UserInfoDTO caricaUserInfo(HttpServletRequest request) {
    ...
    if (!StringUtils.isEmpty(userAuthHeader)) {
      AuthenticationToken<UserInfoDTO> authToken = new CosmoJWTTokenDecoder(
          ...
          LoggerConstants.COSMONOTIFICATIONS_ROOT_LOG_CATEGORY).decodeAndVerify(userAuthHeader,
              UserInfoDTO.class);
