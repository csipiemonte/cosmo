...
import it.csi.cosmo.cosmobe.business.service.impl.ConfigurazioneServiceImpl;
...
public class SecurityInterceptor implements ContainerRequestFilter {
  ...
  private void processAnnotationScopesRequirements(Secured annotation) {
    ...
    if (annotation.authenticated() && !Boolean.FALSE.equals(client.getAnonimo())) {
      handleFailure(POLICY_PREFIX + "AUTHENTICATED");
    }

    if (annotation.guest() && Boolean.FALSE.equals(client.getAnonimo())) {
  ...
  private void handleFailure(Object missing) {
    ...
    LOGGER.warn("logFailed", "accesso negato per il client: " + SecurityUtils.getClientCorrente()
